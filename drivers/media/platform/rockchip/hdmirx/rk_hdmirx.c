...
static u8 edid_init_data_340M[] = {
	...
	/*
	 * CEA-861 extension. 4K50/60 (VIC 96/97) are advertised ONLY in the
	 * Y420VDB below (4:2:0-only), and deliberately NOT in the regular
	 * Video Data Block: this sink advertises no HDMI-Forum VSDB/SCDC and a
	 * 340 MHz max TMDS rate, so it cannot receive 4K50/60 in 4:4:4/RGB
	 * (594 MHz). Listing them in both blocks (as the original EDID did) is
	 * non-compliant and makes some sources drive 594 MHz unscrambled,
	 * which never locks. With 4K60 offered only as 4:2:0 the source sends
	 * 297 MHz YUV420 (-> NV12), which locks on the pre-3.4 Gbps path.
	 */
	0x02, 0x03, 0x2C, 0xF1, 0x4F, 0x07, 0x16, 0x14,
	0x05, 0x01, 0x03, 0x12, 0x13, 0x84, 0x22, 0x1F,
	0x90, 0x5D, 0x5E, 0x5F, 0x23, 0x09, 0x07, 0x07,
	0x83, 0x01, 0x00, 0x00, 0x67, 0x03, 0x0C, 0x00,
	0x30, 0x00, 0x00, 0x44, 0xE3, 0x05, 0x03, 0x01,
	0xE3, 0x0E, 0x60, 0x61, 0x02, 0x3A, 0x80, 0x18,
	0x71, 0x38, 0x2D, 0x40, 0x58, 0x2C, 0x45, 0x00,
	0x20, 0xC2, 0x31, 0x00, 0x00, 0x1E, 0x00, 0x00,
...
static void hdmirx_cec_state_reconfiguration(struct rk_hdmirx_dev *hdmirx_dev, bool en)
{
	...
	hdmirx_update_bits(hdmirx_dev, CEC_CONFIG, RX_AUTO_DRIVE_ACKNOWLEDGE, RX_AUTO_DRIVE_ACKNOWLEDGE);
...
int hdmirx_write_edid(struct rk_hdmirx_dev *hdmirx_dev, struct v4l2_edid *edid, bool hpd_up)
{
	...
	/*
		...
		https://arc.net/l/quote/crszsgia
	*/
	msleep(100);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct rk_hdmirx_dev *hdmirx_dev = container_of(dwork, struct rk_hdmirx_dev, delayed_work_heartbeat);
	queue_work_on(hdmirx_dev->wdt_cfg_bound_cpu,  system_highpri_wq, &hdmirx_dev->work_wdt_config);
	schedule_delayed_work_on(hdmirx_dev->bound_cpu, &hdmirx_dev->delayed_work_heartbeat, HZ);
}
				    ...
				    struct rk_hdmirx_dev *hdmirx_dev)
{
	...
	debugfs_create_file("ctrl", 0600, hdmirx_dev->debugfs_dir, hdmirx_dev, &hdmirx_ctrl_fops);
	debugfs_create_file("phy", 0600, hdmirx_dev->debugfs_dir, hdmirx_dev, &hdmirx_phy_fops);
	debugfs_create_file("status", 0600, hdmirx_dev->debugfs_dir, hdmirx_dev, &hdmirx_status_fops);
}
...
static int hdmirx_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(dev, irq, NULL, hdmirx_dma_irq_handler, IRQF_ONESHOT, RK_HDMIRX_DRVNAME"-dma", hdmirx_dev);
	if (ret) {
	...
	hdmirx_dev->detect_tx_5v_ctrl = v4l2_ctrl_new_std(hdl, NULL, V4L2_CID_DV_RX_POWER_PRESENT, 0, 1, 0, 0);

	/* custom controls */
	hdmirx_dev->audio_sampling_rate_ctrl = v4l2_ctrl_new_custom(hdl, &hdmirx_ctrl_audio_sampling_rate, NULL);
	if (hdmirx_dev->audio_sampling_rate_ctrl)
		hdmirx_dev->audio_sampling_rate_ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;
	hdmirx_dev->audio_present_ctrl = v4l2_ctrl_new_custom(hdl, &hdmirx_ctrl_audio_present, NULL);
	...
	hdmirx_hdcp_register(dev, hdmirx_dev, hdmirx_dev->hdcp_enable);
...
static int hdmirx_remove(struct platform_device *pdev)
{
	...
	if (hdmirx_dev->cec_notifier)
		cec_notifier_conn_unregister(hdmirx_dev->cec_notifier);
	if (hdmirx_dev->hdcp)
		hdmirx_hdcp_unregister(hdmirx_dev->hdcp);
